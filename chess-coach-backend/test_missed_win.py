...
def test_missed_mate_still_winning_is_missed_win():
    """Move 12: mate available, declined, still +465cp (~84%)."""
    r = classify_move(eval_before_cp=MATE, eval_after_cp=453, best_eval_cp=MATE)
...
def test_missed_win_does_not_count_as_blunder():
    ...
    r = classify_move(eval_before_cp=MATE, eval_after_cp=465, best_eval_cp=MATE)
    assert r["severity"] != "Blunder"
...
def test_declining_mate_into_equality_stays_blunder():
    ...
    r = classify_move(eval_before_cp=MATE, eval_after_cp=100, best_eval_cp=MATE)
